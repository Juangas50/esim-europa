...
const STATUSES: Record<string, { label: string; color: string; bg: string }> = {
  ...
  cancelled:      { label: 'Cancelado',  color: '#64748B', bg: 'rgba(122,122,122,0.15)' },
}
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 mb-5 sm:flex-row sm:items-center">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <button onClick={() => setManaging(null)} style={{ width: 34, height: 34, borderRadius: 9, background: '#fff', border: '1px solid #E2E8F0', color: '#64748B', cursor: 'pointer', fontSize: 18, fontFamily: 'inherit', flexShrink: 0 }}>←</button>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span style={{ background: managing.active ? 'rgba(34,197,94,0.12)' : 'rgba(122,122,122,0.12)', color: managing.active ? '#22C55E' : '#64748B', border: `1px solid ${managing.active ? 'rgba(34,197,94,0.3)' : 'rgba(122,122,122,0.3)'}`, borderRadius: 6, padding: '2px 8px', fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
                {managing.active ? 'Activa' : 'Inactiva'}
              </span>
            </div>
            <div style={{ fontSize: 12, color: '#64748B', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{managing.email}</div>
          </div>
        </div>
        {/* Acciones */}
        {!editing ? (
          <div className="flex gap-2">
            <button onClick={() => setEditing(true)} className="flex-1 sm:flex-none" style={{ padding: '8px 14px', background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, color: '#64748B', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
              ✏️ Editar
            </button>
          ...
          <form onSubmit={handleUpdate} className="flex flex-col gap-2 w-full sm:flex-row sm:items-center">
            ...
            <div className="flex gap-2">
              <button type="submit" className="flex-1" style={{ background: '#C9973A', color: '#1B2F4E', border: 'none', borderRadius: 8, padding: '9px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Guardar</button>
              <button type="button" className="flex-1" onClick={() => setEditing(false)} style={{ background: 'transparent', color: '#64748B', border: '1px solid #E2E8F0', borderRadius: 8, padding: '9px 16px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
            </div>
          </form>
        )}
      </div>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #2D4A72', marginBottom: 22 }}>
        {[
          { id: 'precios', label: '💰 Precios' },
          { id: 'pedidos', label: `📦 Pedidos (${orders.length})` },
        ].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: '10px 18px', background: 'none', border: 'none', borderBottom: `2px solid ${tab === t.id ? '#C9973A' : 'transparent'}`, color: tab === t.id ? '#fff' : '#64748B', fontWeight: tab === t.id ? 700 : 400, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginBottom: -1 }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'precios' && (
        <div>
          <p style={{ color: '#64748B', fontSize: 13, marginBottom: 18 }}>
            Configurá el <strong style={{ color: '#1B2F4E' }}>precio coste</strong> (lo que facturamos) y el <strong style={{ color: '#1B2F4E' }}>PVP que ve la agencia</strong>. Se guarda automáticamente al salir del campo.
          </p>
          {['prepago', 'dataonly'].map(type => (
            <div key={type}>
              ...
              {tariffs.filter(t => t.type === type).map(t => {
                ...
                return (
                  <div key={t.id} style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12, padding: '14px 16px', marginBottom: 8 }}>
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontWeight: 800, fontSize: 14 }}>{t.name}</div>
                      <div style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>{t.data_gb} GB{t.validity_days ? ` · ${t.validity_days} días` : ''}</div>
                    </div>
                    <div className="flex flex-wrap gap-4 items-end">
                      ...
                      <div style={{ textAlign: 'center', minWidth: 56 }}>
                        <div style={{ fontSize: 10, color: '#64748B', fontWeight: 700, textTransform: 'uppercase', marginBottom: 5 }}>Margen</div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <select value={orderFilter} onChange={e => setOrderFilter(e.target.value)}
              style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, padding: '9px 12px', color: '#1B2F4E', fontSize: 13, outline: 'none', fontFamily: 'inherit' }}>
            ...
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
              title="Desde"
              style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, padding: '9px 12px', color: dateFrom ? '#fff' : '#64748B', fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
              title="Hasta"
              style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, padding: '9px 12px', color: dateTo ? '#fff' : '#64748B', fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
            {(dateFrom || dateTo) && (
              <button onClick={() => { setDateFrom(''); setDateTo('') }}
                style={{ background: 'transparent', border: '1px solid #E2E8F0', borderRadius: 8, padding: '9px 12px', color: '#64748B', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                ✕ Limpiar fechas
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button
              ...
              style={{ background: '#C9973A', color: '#1B2F4E', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
            >
          ...
          {filteredOrders.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: '#64748B', fontSize: 13, background: '#F8FAFC', borderRadius: 14, border: '1px solid #E2E8F0' }}>No hay pedidos que coincidan con los filtros</div>
          ) : isMobile ? (
            // Mobile: cards
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filteredOrders.map(o => {
                ...
                return (
                  <div key={o.id} style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12, padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
                      <span style={{ fontSize: 12, color: '#64748B' }}>{o.tariffs?.name}</span>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {filteredOrders.map((o, i) => {
                    const st = STATUSES[o.status] || STATUSES.pending_review
                    return (
                      <tr key={o.id} style={{ borderBottom: i < filteredOrders.length - 1 ? '1px solid #2D4A72' : 'none' }}>
                        <td style={{ padding: '12px 16px', fontSize: 11, color: '#64748B', fontFamily: 'monospace' }}>{o.order_ref}</td>
                        ...
                        <td style={{ padding: '12px 16px', fontSize: 12, color: '#64748B' }}>{o.activation_date || 'Inmediata'}</td>
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <button onClick={() => setAdding(!adding)} style={{ background: '#C9973A', color: '#1B2F4E', border: 'none', borderRadius: 9, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
          + Nueva agencia
        </button>
      </div>

      {adding && (
        <form onSubmit={handleCreate} style={{ background: '#F8FAFC', border: '1px solid rgba(230,0,0,0.3)', borderRadius: 14, padding: 20, marginBottom: 18 }}>
          <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 16 }}>Nueva agencia</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            <div>
              <label style={{ fontSize: 11, color: '#64748B', fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', display: 'block', marginBottom: 5 }}>Nombre *</label>
              <input required style={inp} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Viajes Ejemplo" />
            </div>
            <div>
              <label style={{ fontSize: 11, color: '#64748B', fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', display: 'block', marginBottom: 5 }}>Email *</label>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:justify-end">
            <button type="button" onClick={() => setAdding(false)} className="sm:w-auto" style={{ background: 'transparent', color: '#64748B', border: '1px solid #E2E8F0', borderRadius: 9, padding: '10px 18px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
            <button type="submit" className="sm:w-auto" style={{ background: '#C9973A', color: '#1B2F4E', border: 'none', borderRadius: 9, padding: '10px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Crear agencia</button>
          </div>
        </form>
      )}

      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {agencies.length === 0 && <div style={{ color: '#64748B', fontSize: 13, padding: 20 }}>No hay agencias. Creá la primera.</div>}
        {agencies.map(a => (
          <div key={a.id} style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 14, padding: '16px' }}>
            <div className="flex items-center gap-4 mb-3">
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(110,193,228,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>🏢</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 800, fontSize: 15, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.name}</div>
                <div style={{ fontSize: 12, color: '#64748B', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.email}</div>
              </div>
              <span style={{ background: a.active ? 'rgba(34,197,94,0.12)' : 'rgba(122,122,122,0.12)', color: a.active ? '#22C55E' : '#64748B', border: `1px solid ${a.active ? 'rgba(34,197,94,0.3)' : 'rgba(122,122,122,0.3)'}`, borderRadius: 6, padding: '3px 8px', fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
                {a.active ? 'Activa' : 'Inactiva'}
              </span>
            </div>
            <button onClick={() => handleManage(a)} style={{ width: '100%', padding: '9px 0', background: '#fff', border: '1px solid #E2E8F0', borderRadius: 8, color: '#64748B', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit', fontWeight: 600 }}>
              Gestionar →
            </button>
          </div>
        ))}
...
function PriceInput({ label, defaultValue, onSave }: { label: string; defaultValue: number; onSave: (val: number) => void }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 10, color: '#64748B', fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 5 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <span style={{ background: '#fff', border: '1px solid #E2E8F0', borderRight: 'none', borderRadius: '8px 0 0 8px', padding: '8px 10px', fontSize: 13, color: '#64748B' }}>$</span>
        <input type="number" min="0" step="0.01" value={val} placeholder="0"
          ...
          style={{ width: 70, background: '#fff', border: '1px solid #E2E8F0', borderRadius: '0 8px 8px 0', padding: '8px 10px', color: '#1B2F4E', fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
      </div>
    </div>
  )
}